...
ChatLogic::ChatLogic()
{
    // no need to instantiate _chatBot since it is points to ChatBot instance
    // passed on the nodes
    ...
    // GUI _chatBot->SetChatLogicHandle(this);
}

ChatLogic::~ChatLogic()
{
    // no need to delete _chatbot since it is on stack
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
                ...
                if (idToken != tokens.end()) {
                    ...
                    if (type->second == "EDGE") {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end()) {
                            ...
                            (*parentNode)
                                ->AddEdgeToChildNode(
                                    std::move(edge)); // give ownership of edge to parent
                        }
                    }
                } else {
    ...
    //create a chatbot instance which will be moved among graph nodes
    ChatBot chat_bot("../images/chatbot.png"); //instantiate with image
    chat_bot.SetChatLogicHandle(this); //set the logic handle to the logic address
    chat_bot.SetRootNode(rootNode); //set the root node
    rootNode->MoveChatbotHere(std::move(chat_bot)); //transfer the chatbot to the root node
